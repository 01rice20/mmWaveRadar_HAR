...
from model.function import set_seed
...
class ECAAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        self.conv = (nn.Conv1d(1, 1, kernel_size=kernel, padding=int(kernel/2), bias=False)).cuda()
        y = self.avg_pool(x)
...
class ResidualAttention(nn.Module):
    ...
    def __init__(self, channel, la=0.2):
        super().__init__()
        self.la = la
        self.fc = nn.Conv2d(in_channels=channel, out_channels=channel*2, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, x):
        ...
        # Without Flatten
        print("before resatt: ", x.shape)
        y_raw = self.fc(x)
        y_avg = torch.mean(y_raw)
        print("y_avg: ", y_avg.shape)
        y_max = torch.max(y_raw)[0]
        print("y_max: ", y_max.shape)
        x = y_avg + self.la * y_max
        print("after resatt: ", x.shape)
        # score = x + concate
...
class DAT(nn.Module):
    ...
    def __init__(self, img_size=128, patch_size=4, num_classes=11, expansion=4,
